/*!
        @file    test_HMC_Clover_SF_RHMC_Nf2p1.cpp
...
namespace Test_HMC_Clover_SF {
  const std::string test_name = "HMC.Clover_SF.RHMC_Nf2p1";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Clover_SF_RHMC_Nf2p1.yaml";
  }
  ...
  int RHMC_Nf2p1(void)
  {
    ...
    const Parameters params_test        = params_all.lookup("Test_HMC_Clover_SF");
    ...
    //-- N_f=2 part
    unique_ptr<Fopr> fopr_Nf2(new Fopr_Clover_SF());
    fopr_Nf2->set_parameters(params_fopr_Nf2);
    unique_ptr<Force> force_fopr_Nf2(new Force_F_Clover_SF());
    ...
    //-- N_f=1 part
    unique_ptr<Fopr> fopr_Nf1(new Fopr_Clover_SF());
    fopr_Nf1->set_parameters(params_fopr_Nf1);

    unique_ptr<Force> force_fopr_Nf1(new Force_F_Clover_SF());
    force_fopr_Nf1->set_parameters(params_fopr_Nf1);

    unique_ptr<Fopr> fopr_r1(Fopr::New("Rational_SF", fopr_Nf1));
    fopr_r1->set_parameters(params_rational_H);
    unique_ptr<Fopr> fopr_langev(Fopr::New("Smeared", fopr_r1, dr_smear));

    unique_ptr<Fopr> fopr_r2(Fopr::New("Rational_SF", fopr_Nf1));
    ...
    unique_ptr<Force> force_fopr_MD(
      new Force_F_Smeared(force_fopr_r2,
                          force_smear, dr_smear));

    unique_ptr<Action> action_F_Nf1(
      new Action_F_Rational_SF(fopr_langev,
                               fopr_H,
                               force_fopr_MD));
    ...
    actions.append(level_action[1], action_F_Nf1);
    actions.append(level_action[2], action_G);
    ...
    vout.general(vl, "RHMC start: Ntraj = %d\n", Ntraj);
