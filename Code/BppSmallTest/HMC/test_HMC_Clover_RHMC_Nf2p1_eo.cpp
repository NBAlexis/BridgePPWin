/*!
        @file    test_HMC_Clover_RHMC_Nf2p1_eo.cpp
...
namespace Test_HMC_Clover {
  const std::string test_name = "HMC.Clover.RHMC_Nf2p1_eo";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Clover_RHMC_Nf2p1.yaml";
  }
  ...
  int RHMC_Nf2p1_eo(void)
  {
    ...
    if ((str_solver_Nf2_MD_type == "CG") || (str_solver_Nf2_H_type == "CG") ||
        (str_solver_Nf2_MD_prec_type == "CG") || (str_solver_Nf2_H_prec_type == "CG")) {
    ...
    unique_ptr<Force> force_fopr_Nf2(new Force_F_Clover_Nf2(str_gmset_type));
    force_fopr_Nf2->set_parameters(params_fopr_Nf2);

    unique_ptr<Fopr>  fopr_smear_Nf2(Fopr::New("Smeared", fopr_Nf2, dr_smear));
    unique_ptr<Force> force_fopr_smear_Nf2(new Force_F_Smeared(force_fopr_Nf2, force_smear, dr_smear));

    //- NB1 fopr_Nf2_eo->set_config is performed in Action_F_Standard_eo.
    //  NB2 fopr_Nf2_eo->set_mode   is performed in fprop.
    unique_ptr<Fopr> fopr_Nf2_eo(Fopr::New(str_fopr_Nf2_type + "_eo", str_gmset_type));
    fopr_Nf2_eo->set_parameters(params_fopr_Nf2);

    unique_ptr<Fopr> fopr_smear_Nf2_eo(Fopr::New("Smeared_eo", fopr_Nf2_eo, dr_smear));

    unique_ptr<Solver> solver_Nf2_eo_MD(Solver::New(str_solver_Nf2_MD_type, fopr_smear_Nf2_eo));
    solver_Nf2_eo_MD->set_parameters(params_solver_Nf2_MD);
    unique_ptr<Fprop> fprop_Nf2_eo_MD(new Fprop_Standard_eo(solver_Nf2_eo_MD));

    unique_ptr<Solver> solver_Nf2_eo_H(Solver::New(str_solver_Nf2_H_type, fopr_smear_Nf2_eo));
    solver_Nf2_eo_H->set_parameters(params_solver_Nf2_H);
    unique_ptr<Fprop> fprop_Nf2_eo_H(new Fprop_Standard_eo(solver_Nf2_eo_H));
    ...
    unique_ptr<Force> force_fopr_Nf2_prec(new Force_F_Clover_Nf2(str_gmset_type));
    force_fopr_Nf2_prec->set_parameters(params_fopr_Nf2_prec);

    unique_ptr<Fopr>  fopr_smear_Nf2_prec(Fopr::New("Smeared", fopr_Nf2_prec, dr_smear));
    unique_ptr<Force> force_fopr_smear_Nf2_prec(new Force_F_Smeared(force_fopr_Nf2_prec, force_smear, dr_smear));

    //- NB1 fopr_Nf2_eo_prec->set_config is performed in Action_F_Standard_eo.
    //  NB2 fopr_Nf2_eo_prec->set_mode   is performed in fprop.
    unique_ptr<Fopr> fopr_Nf2_eo_prec(Fopr::New(str_fopr_Nf2_prec_type + "_eo", str_gmset_type));
    fopr_Nf2_eo_prec->set_parameters(params_fopr_Nf2_prec);

    unique_ptr<Fopr> fopr_smear_Nf2_eo_prec(Fopr::New("Smeared_eo", fopr_Nf2_eo_prec, dr_smear));

    unique_ptr<Solver> solver_Nf2_eo_MD_prec(Solver::New(str_solver_Nf2_MD_type, fopr_smear_Nf2_eo_prec));
    solver_Nf2_eo_MD_prec->set_parameters(params_solver_Nf2_MD);
    unique_ptr<Fprop> fprop_Nf2_eo_MD_prec(new Fprop_Standard_eo(solver_Nf2_eo_MD_prec));

    unique_ptr<Solver> solver_Nf2_eo_H_prec(Solver::New(str_solver_Nf2_H_type, fopr_smear_Nf2_eo_prec));
    solver_Nf2_eo_H_prec->set_parameters(params_solver_Nf2_H);
    unique_ptr<Fprop> fprop_Nf2_eo_H_prec(new Fprop_Standard_eo(solver_Nf2_eo_H_prec));


    unique_ptr<Action> action_F_Nf2_prec(
      new Action_F_Standard_eo(fopr_smear_Nf2_prec, force_fopr_smear_Nf2_prec,
                               fprop_Nf2_eo_MD_prec, fprop_Nf2_eo_H_prec));

    unique_ptr<Action> action_F_Nf2_d(
      new Action_F_Ratio_eo(fopr_smear_Nf2_prec, force_fopr_smear_Nf2_prec,
                            fopr_smear_Nf2, force_fopr_smear_Nf2,
                            fprop_Nf2_eo_H_prec,
                            fprop_Nf2_eo_MD, fprop_Nf2_eo_H));
    ...
    unique_ptr<Force> force_fopr_Nf1(new Force_F_Clover_Nf2(str_gmset_type));
    ...
    unique_ptr<Fopr> fopr_smear_Nf1_langev(Fopr::New("Smeared", fopr_r1, dr_smear));
    ...
    unique_ptr<Fopr> fopr_smear_Nf1_H(Fopr::New("Smeared", fopr_r2, dr_smear));

    unique_ptr<Force> force_fopr_Nf1_MD(new Force_F_Rational(fopr_Nf1, force_fopr_Nf1));
    force_fopr_Nf1_MD->set_parameters(params_rational_MD);
    unique_ptr<Force> force_fopr_smear_Nf1_MD(new Force_F_Smeared(force_fopr_Nf1_MD, force_smear, dr_smear));


    unique_ptr<Action> action_F_Nf1(
      new Action_F_Rational(fopr_smear_Nf1_langev, fopr_smear_Nf1_H, force_fopr_smear_Nf1_MD));
