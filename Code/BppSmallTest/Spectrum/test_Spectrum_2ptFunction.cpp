/*!
        @file    test_Spectrum_2ptFunction.cpp
...
/*!
    ...
    Adopted new parameter handlings
                               [26 Jun 2016 T.Aoyama]
    Add shift_origin of gauge config
                               [21 May 2017 Y.Namekawa]
...
/*
  Description of Parameters
    ...
    Projection:
    Smear:
    Director_Smear: { parameters for smearing }
...
namespace Test_Spectrum {
  ...
  // int hadron_2ptFunction_TMWilson()
  // {
  //   return hadron_2ptFunction("test_Spectrum_TMWilson_Hadron2ptFunction.yaml");
  // }
  ...
  {
    return hadron_2ptFunction("test_Spectrum_Wilson_Hadron2ptFunction_ShiftOrigin.yaml");
  }
  ...
  namespace {
    ...
    //   "Spectrum.TMWilson.Hadron2ptFunction",
    //   hadron_2ptFunction_TMWilson
    //   );
    ...
    static const bool is_registered_Wilson_ShiftOrigin = TestManager::RegisterTest(
      "Spectrum.Wilson.Hadron2ptFunction_ShiftOrigin",
      hadron_2ptFunction_Wilson_ShiftOrigin
      );

    static const bool is_registered_Wilson_WallSource = TestManager::RegisterTest(
    ...
    for (int iq = 0; iq < N_quark; ++iq) {
      ...
      for (int ispin = 0; ispin < Nd; ++ispin) {
        for (int icolor = 0; icolor < Nc; ++icolor) {
          ...
          int    Nconv;
          double diff;
          fprop_lex[iq]->invert_D(sq[iq][i_cd], b, Nconv, diff);

          Field_F y(b);
          fopr[iq]->set_mode("D");
          fopr[iq]->mult(y, sq[iq][i_cd]); // y  = fopr[iq]->mult(sq[iq][i_cd]);
          axpy(y, -1.0, b);                // y -= b;
