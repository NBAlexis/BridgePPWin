/*!
        @file    $Id:: test_Spectrum_2ptFunction_eo_withFileIO.cpp #$
...
namespace Test_Spectrum {
  ...
  int calculate_hadron_correlator_eo_withFileIO(const std::string& filename_input)
  {
    ...
    for (int iq = 0; iq < N_quark; ++iq) {
      savefile_base[iq] = params_all.lookup(Filename("Quark_{id}").format(iq + 1)).get_string("temporary_filename_base");
    }
    ...
    for (int iq = 0; iq < N_quark; ++iq) {
      ...
      str_source_type[iq] = params_all.lookup(Filename("Quark_{id}").format(iq + 1)).lookup("Source").get_string("source_type");
