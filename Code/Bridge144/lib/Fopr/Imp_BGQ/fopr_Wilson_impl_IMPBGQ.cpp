...
#include "fopr_Wilson_impl_SU_N.inc"
  ...
  const std::string Fopr_Wilson::class_name = "Imp_BGQ::Fopr_Wilson";
//====================================================================
...
//====================================================================
  void Fopr_Wilson::set_mode(std::string mode)
  {
    ...
    if (m_mode == "D") {
      ...
      vout.crucial(m_vl, "Error at %s: input mode is undefined in Fopr_Wilson\n", class_name.c_str());
      exit(EXIT_FAILURE);
    }
  }
