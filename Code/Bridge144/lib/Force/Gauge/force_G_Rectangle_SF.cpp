...
/*!
        @file    $Id:: force_G_Rectangle_SF.cpp #$
...
namespace {
  ...
  bool init = Force_G::Factory::Register("Force_G_Rectangle_SF", create_object);
}
#endif
...
*/
void Force_G_Rectangle_SF::set_parameters(double beta, double c_plaq, double c_rect,
...
void Force_G_Rectangle_SF::force_core(Field& force)
{
  ...
  Mat_SU_N vmat(m_Nc), wmat(m_Nc), cmat(m_Nc);
  Mat_SU_N wzero(m_Nc);
  wzero.zero();
