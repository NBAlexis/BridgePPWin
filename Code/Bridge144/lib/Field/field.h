...
/*!
This class BAPI defines field-type quantity which has three
...
extern double BAPI dot(const Field& y, const Field& x);

//! dot(y[j], x[k]) := y[j]^T x[k]
extern double BAPI dot(const Field& y, const int exy, const Field& x, const int exx);

//! dotc(y, x) := y^dag x
//!   x, y may be real or complex vectors. (must be of same element type)
extern dcomplex BAPI dotc(const Field& y, const Field& x);

//! dotc(y, j, x, k) := y[j]^dag x[k]
extern dcomplex BAPI dotc(const Field& y, const int exy, const Field& x, const int nexx);

//! axpy(y, a, x):  y := a * x + y
extern void BAPI axpy(Field& y, const double a, const Field& x);

//! axpy(y, j, a, x, k):  y[j] := a * x[k] + y[j]
extern void BAPI axpy(Field& y, const int exy, const double a, const Field& x, const int exx);

//! axpy(y, a, x):  y := a * x + y
//!   fails if x or y is real vector
extern void BAPI axpy(Field& y, const dcomplex a, const Field& x);

//! axpy(y, j, a, x, k):  y[j] := a * x[k] + y[j]
extern void BAPI axpy(Field& y, const int exy, const dcomplex a, const Field& x, const int exx);

//! scal(x, a): x = a * x
extern void BAPI scal(Field& x, const double a);

//! scal(x, k, a): x[k] = a * x[k]
extern void BAPI scal(Field& x, const int exx, const double a);

//! scal(x, a): x = a * x
//!   fails if x is a real vector
extern void BAPI scal(Field& x, const dcomplex a);

//! scal(x, k, a): x[k] = a * x[k]
extern void BAPI scal(Field& x, const int exx, const dcomplex a);

//! copy(y, x): y = x
extern void BAPI copy(Field& y, const Field& x);

//! copy(y, j, x, k): y[j] = x[k]
extern void BAPI copy(Field& y, const int exy, const Field& x, const int exx);

//! aypx(y, a, x):  y := a * y + x
extern void BAPI aypx(const double a, Field& y, const Field& x);

//! aypx(y, a, x):  y := a * y + x
//!   fails if x or y is real vector
extern void BAPI aypx(const dcomplex a, Field& y, const Field& x);
...
extern void BAPI report_field_stat(const Bridge::VerboseLevel vl, const std::string& msg, const Field& f);
