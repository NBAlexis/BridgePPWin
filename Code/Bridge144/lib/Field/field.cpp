...
namespace {
                      ...
                      const int size)
  {
    ...
    is = size * i_thread / Nthread;
    ns = size * (i_thread + 1) / Nthread;
  }
}
...
dcomplex dotc(const Field& y, const Field& x)
{
  ...
  if ((y.field_element_type() == Field::COMPLEX) &&
  ...
  } else if ((y.field_element_type() == Field::REAL) &&
             (y.field_element_type() == Field::REAL)) {
...
void axpy(Field& y, const dcomplex a, const Field& x)
{
  if (imag(a) == 0.0) {
    return axpy(y, real(a), x);
             ...
             (x.field_element_type() == Field::REAL)) {
...
{
  if (x.field_element_type() == Field::REAL) {
    vout.crucial("Error at %s: real vector and complex parameter.\n", __func__);
    exit(EXIT_FAILURE);
