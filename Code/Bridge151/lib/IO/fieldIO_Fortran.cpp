...
void FieldIO_Fortran::read_file(Field *v, const std::string filename)
{
  ...
  if (Communicator::is_primary()) {
    ...
    vtmp.reset(nin_field, (const int)Lvol, nex_field);
    ...
    for (int j = 0; j < nex_file; ++j) {
      for (long_t isite = 0; isite < Lvol; ++isite) {
        ...
        for (int i = 0; i < nin_file; ++i) {
          ...
          vtmp.set(s, (const int)isite, t, ptr[i]);
        }
      }
    }
...
void FieldIO_Fortran::write_file(Field *v, const std::string filename)
{
  ...
  if (Communicator::is_primary()) {
    ...
    const uint32_t flen = static_cast<const uint32_t>(sizeof(double) * count);
    ...
    for (int j = 0; j < nex_file; ++j) {
      for (long_t isite = 0; isite < Lvol; ++isite) {
        ...
        for (int i = 0; i < nin_file; ++i) {
          ...
          ptr[i] = vtmp.cmp(s, (const int)isite, t);
        }
...
void FieldIO_Fortran::read_file(std::vector<Field *>& vv, const std::string& filename)
{
  ...
  const int nin_field = vv[0]->nin();
  const int nex_field = vv[0]->nex();
    ...
    for (size_t i = 0, nrec = vv.size(); i < nrec; ++i) {
      ...
      for (int j = 0; j < nex_file; ++j) {
        for (long_t isite = 0; isite < Lvol; ++isite) {
          ...
          for (int i1 = 0; i1 < nin_file; ++i1) {
            int s, t;
            m_format->file_to_field(s, t, i1, j);

            vtmp.set(s, (const int)isite, t, ptr[i1]);
          }
        }
      }
      ...
      for (int j = 0; j < nex_file; ++j) {
        for (long_t isite = 0; isite < Lvol; ++isite) {
          ...
          for (int i1 = 0; i1 < nin_file; ++i1) {
            ...
            ptr[i1] = vtmp.cmp(s, (const int)isite, t);
          }
