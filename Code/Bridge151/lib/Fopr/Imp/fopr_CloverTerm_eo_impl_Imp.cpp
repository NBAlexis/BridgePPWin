...
//====================================================================
                                              ...
                                              const Field& f, const int ieo)
  {
    ...
    const int is       = m_Nvol2 * i_thread / Nthread;
    const int ns       = m_Nvol2 * (i_thread + 1) / Nthread;
    ...
    for (int site = is; site < ns; ++site) {
      for (int icd = 0; icd < m_Nc * Nd2; ++icd) {
        ...
        for (int jd = 0; jd < m_Nd; ++jd) {
          ...
          v4[iv2]     += mult_uv_r(&csw_inv[ig], &v3[iv], m_Nc);
          v4[iv2 + 1] += mult_uv_i(&csw_inv[ig], &v3[iv], m_Nc);
        }
      }
...
//====================================================================
  void Fopr_CloverTerm_eo::mult_isigma(Field_F& v,
                                       const Field_F& w, const int mu, const int nu)
  {
    assert(mu != nu);
...
//====================================================================
                                             ...
                                             const int mu, const int nu)
  {
    ...
    Field_G v2local;
    for (int site = 0; site < m_Nvol; ++site) {
      v2local.set_mat(site, 0, Umu.mat(site) * Cdn.mat_dag(site));
    }
    ...
    for (int site = 0; site < m_Nvol; ++site) {
      v2local.set_mat(site, 0, Cdn.mat_dag(site) * Umu.mat(site));
    }

    axpy(v, -1.0, v2local);

    m_shift_eo.forward(v2local, v, mu);
...
//====================================================================
  void Fopr_CloverTerm_eo::trSigmaInv(Field_G& tr_sigma_inv,
                                      const int mu, const int nu)
  {
