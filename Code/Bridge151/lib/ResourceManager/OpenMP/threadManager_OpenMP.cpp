...
#pragma omp parallel
  ...
  m_darray_reductionF.resize(m_Nthread);
}
...
void ThreadManager_OpenMP::reduce_sum_global(float& a,
                                             const int i_thread, const int Nthread)
{
  m_darray_reductionF[i_thread] = a;
  barrier(Nthread);
...
#pragma omp flush
#else
#pragma omp flush (m_darray_reductionF)
#endif

    for (int i = 0; i < Nthread; ++i) {
      b += m_darray_reductionF[i];
    }
    b = Communicator::reduce_sum(b);
...
#pragma omp flush (m_darray_reductionF)
    ...
    m_darray_reductionF[0] = b;
  }

#pragma omp barrier

  a = m_darray_reductionF[0];
