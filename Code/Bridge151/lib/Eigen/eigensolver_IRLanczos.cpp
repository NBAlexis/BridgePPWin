...
void Eigensolver_IRLanczos::set_parameters(const std::string& sort_type,
                                           const int Nk, const int Np,
                                           const int Niter_eigen, const double Enorm_eigen,
...
//====================================================================
void Eigensolver_IRLanczos::set_parameters(const int Nk, const int Np,
                                           const int Niter_eigen, double Enorm_eigen,
                                           double Vthreshold)
{
                                  ...
                                  int& Nsbt, int& Nconv, const Field& b)
{
  const int    Nk          = m_Nk;
  const int    Np          = m_Np;
  const int    Niter_eigen = m_Niter_eigen;
  const double Enorm_eigen = m_Enorm_eigen;
  const double Vthreshold  = m_Vthreshold;
  ...
  //- Set initial vector
  vk[0].set(1.0);
  const double vnorm = dot(vk[0], vk[0]);
  ...
  for (int iter = 0; iter < Niter_eigen; ++iter) {
    ...
    double beta_k = dot(f, f);
    beta_k = sqrt(beta_k);

    vout.detailed(m_vl, " beta(k) = %20.14f\n", beta_k);


    const double beta_r = 1.0 / beta_k;
    vk[k2] = f;
    scal(vk[k2], beta_r);
    TDb[k2 - 1] = beta_k;
    ...
    for (int i = 0; i < Nk; ++i) {
      m_fopr->mult(v, B[i]);
      const double vnum = dot(B[i], v);
      const double vden = dot(B[i], B[i]);
...
//====================================================================
void Eigensolver_IRLanczos::step(const int Nm, const int k,
                                 std::vector<double>& TDa, std::vector<double>& TDb,
                                 std::vector<Field>& vk, Field& w)
{
  if (k >= Nm) {
...
void Eigensolver_IRLanczos::schmidt_orthogonalization(Field& w,
                                                      const std::vector<Field>& vk, const int k)
{
  for (int j = 0; j < k; ++j) {
...
//====================================================================
void Eigensolver_IRLanczos::setUnit_Qt(const int Nm, std::vector<double>& Qt)
{
    ...
    for (int j = kmax - 1; j >= kmin; --j) {
      ...
      if (fabs(TDb[j - 1]) + dds > dds) {
        ...
        for (int k = 0; k < kmax - 1; ++k) {
          double dds1 = fabs(TDa[k]) + fabs(TDa[k + 1]);

          if (fabs(TDb[k]) + dds1 > dds1) {
            kmin = k + 1;
                                  ...
                                  const double Dsh, const int kmin, const int kmax)
{
  ...
  const double Fden = 1.0 / sqrt((TDa[k] - Dsh) * (TDa[k] - Dsh)
                                 + TDb[k] * TDb[k]);
  const double c = (TDa[k] - Dsh) * Fden;
  const double s = -TDb[k] * Fden;

  const double tmpa1 = TDa[k];
  const double tmpa2 = TDa[k + 1];
  const double tmpb  = TDb[k];
  ...
  for (int k1 = kmin; k1 < kmax - 1; ++k1) {
    double fden = 1.0 / sqrt(x * x + TDb[k1 - 1] * TDb[k1 - 1]);
    double c1    = TDb[k1 - 1] * fden;
    double s1    = -x * fden;

    double tmpa3 = TDa[k1];
    double tmpa4 = TDa[k1 + 1];
    double tmpb1  = TDb[k1];
    TDa[k1]     = c1 * c1 * tmpa3 + s1 * s1 * tmpa4 - 2.0 * c1 * s1 * tmpb1;
    TDa[k1 + 1] = s1 * s1 * tmpa3 + c1 * c1 * tmpa4 + 2.0 * c1 * s1 * tmpb1;
    TDb[k1]     = c1 * s1 * (tmpa3 - tmpa4) + (c1 * c1 - s1 * s1) * tmpb1;
