...
{
  LOG;
  return MPI_Bcast(data, static_cast<int>(size), MPI_BYTE, sender, m_comm);
}
...
int Communicator_impl::Base::exchange(size_t size, void *recv_buf, void *send_buf, int idir, int ipm, int itag)
{
  ...
  return MPI_Sendrecv(
    send_buf, static_cast<int>(size), MPI_BYTE, p_send, tag_send,
    recv_buf, static_cast<int>(size), MPI_BYTE, p_recv, tag_recv,
    m_comm, &status);
}
...
int Communicator_impl::Base::send_1to1(size_t size, void *recv_buf, void *send_buf, int send_to, int recv_from, int tag)
{
  ...
  if (send_to == recv_from) {
    ...
    if (self() == recv_from)
      MPI_Send(send_buf, static_cast<int>(size), MPI_BYTE, send_to, tag, m_comm);

    if (self() == send_to)
      MPI_Recv(recv_buf, static_cast<int>(size), MPI_BYTE, recv_from, tag, m_comm, &status);
  }
...
int Communicator_impl::broadcast_string(int count, string& data, int sender)
{
  ...
  // do broadcast.
  int retv = MPI_Bcast((void *)buf, static_cast<int>(size), MPI_BYTE, sender, m_comm);
